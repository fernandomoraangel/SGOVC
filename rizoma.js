...
async function getGraphDataForD3() {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            if (!rawData) {
                console.warn("No se encontraron datos de OVCs en localStorage ('ovcData').");
                resolve({ nodes: [], links: [] });
                return;
            }
            ...
            if (!Array.isArray(ovcs) || ovcs.length === 0) {
                console.warn("Los datos en 'ovcData' no son un array válido o están vacíos.");
                resolve({ nodes: [], links: [] });
                return;
            }
            ...
            const nodes = ovcs.map(ovc => ({
                ...
                primeraAsignatura: (ovc.asignaturas && ovc.asignaturas.length > 0) ? ovc.asignaturas[0] : 'Sin Asignatura',
                ...
                if (ovc.relaciones && Array.isArray(ovc.relaciones)) {
                    ovc.relaciones.forEach(rel => {
                        ...
                        if (sourceId && targetId) {
                            ...
                            if (rel.direccion === 'saliente') {
                            ...
                            } else { // Incluye bidireccional o no especificado
                                ...
                                if (rel.direccion === 'bidireccional') {
                                     ...
                                     console.warn(`Dirección de relación no especificada o desconocida para ${ovc.titulo} -> ${targetTitle}. Asumiendo 'saliente'.`);
                                }
                            }
                            ...
                            if (finalSourceId && finalTargetId) {
                                links.push({
                                 ...
                                 console.warn(`No se pudo crear enlace: ${rel.direccion === 'entrante' ? targetTitle : ovc.titulo} -> ${rel.direccion === 'entrante' ? ovc.titulo : targetTitle}. IDs: ${finalSourceId}, ${finalTargetId}`);
                            }

                        } else {
                            console.warn(`No se pudo encontrar ID para OVC en relación: ${targetTitle} (relacionado con ${ovc.titulo})`);
                        }
                    });
                }
            });
...
async function verRizoma(containerId) {
    ...
    try {
        ...
        container.html(`<div style="color: red; padding: 10px;">Error al cargar los datos: ${error.message}</div>`);
        return;
    }
    ...
    const colorScale = d3.scaleOrdinal(d3.schemeTableau10)
                         .domain(asignaturas); // Usar las asignaturas únicas para el dominio

    // --- Definición de formas por semestre ---
    const semestres = [...new Set(nodes.map(d => d.semestre))].filter(s => s !== 'Sin Semestre').sort();
    const shapes = [
    ...
    function ticked() {
        ...
        nodeLabel.attr("x", d => d.x + effectiveRadius + 2)
                 .attr("y", d => d.y);
        ...
        if (tickCounter % 50 === 0 && nodes.length > 0) {
             console.log(`DEBUG Tick ${tickCounter}: Nodo 0 pos: (${nodes[0].x?.toFixed(1)}, ${nodes[0].y?.toFixed(1)})`);
        }
        tickCounter++;
    }
    ...
    function linkLine(d) {
        if (d.source.x == null || d.source.y == null || d.target.x == null || d.target.y == null) {
        ...
        if (distance === 0) return `M${d.source.x},${d.source.y}L${d.target.x},${d.target.y}`; // Evitar división por cero
    ...
    nodes.forEach(node => {
        const validWidth = typeof width === 'number' && isFinite(width) ? width : 600;
        const validHeight = typeof height === 'number' && isFinite(height) ? height : 500;
    ...
    const simulation = d3.forceSimulation(nodes)
        .force("link", d3.forceLink(links).id(d => d.id).distance(120)) // Aumentar distancia
    ...
    const link = g.append("g")
        ...
        .data(links)
        .enter().append("path") // Usar path para poder aplicar marcadores
        .attr("class", "link")
        .attr("stroke", "#999")
        ...
        .attr("marker-start", d => d.direccion === 'bidireccional' ? "url(#start)" : null); // Añadir marcador inicial si es bidireccional
    ...
    const linkLabel = g.append("g")
        ...
        .data(links)
        .enter().append("text") // Añadimos el elemento text
    ...
    const node = g.append("g")
        ...
        .enter().append("path")
        .attr("class", "node")
        .attr("d", d => { // Asignar forma según semestre
        ...
        .on("click", function(event, d) { // Añadir manejador de clic
            event.stopPropagation(); // Detener propagación para no interferir con zoom/pan
            if (typeof showSummaryModal === 'function') { // Usar el nombre correcto de la función
    ...
    // Añadir tooltips
    node.append("title")
        .text(d => `${d.label}\nSemestre: ${d.semestre}\nAsignatura: ${d.primeraAsignatura}\nDesc: ${d.descripcion || 'N/A'}`);
    ...
    setTimeout(() => {
        if (nodes.length > 0 && g.node()) {
            try {
                ...
                if (!bounds || typeof bounds.width !== 'number' || typeof bounds.height !== 'number' || !isFinite(bounds.width) || !isFinite(bounds.height) || bounds.width <= 0 || bounds.height <= 0 || !fullWidth || !fullHeight) {
                    console.warn("DEBUG: No se pudieron obtener límites válidos para el zoom inicial o dimensiones del contenedor cero.", { bounds, fullWidth, fullHeight });
                     ...
                     const defaultTransform = d3.zoomIdentity.translate(defaultTranslate[0], defaultTranslate[1]).scale(defaultScale);
                ...
                const scale = Math.min(1.5, 0.9 * Math.min(fullWidth / width, fullHeight / height)); // Limitar zoom máximo a 1.5x
                ...
                console.log("DEBUG: Zoom inicial aplicado.", { scale, translate, bounds });
             ...
             console.log("DEBUG: No hay nodos o el grupo 'g' no está listo para calcular el zoom inicial.");
        }
    ...
    if (asignaturas.length > 0) {
        ...
        asignaturas.forEach((asignatura, i) => { // Mostrar todas las asignaturas en la leyenda
    ...
    if (semestres.length > 0) {
        ...
        semestres.slice(0, shapes.length).forEach((semestre, i) => { // Iterar sobre semestres con forma asignada
     ...
     node.on('mouseover', function(event, d) {
        ...
        link.style('stroke-opacity', l => (l.source === d || l.target === d) ? 1 : 0.1)
            .style('stroke', l => (l.source === d || l.target === d) ? 'black' : '#999')
            .style('stroke-width', l => (l.source === d || l.target === d) ? 2.5 : 1.5);
        linkLabel.style('opacity', l => (l.source === d || l.target === d) ? 1 : 0.3);
        node.style('opacity', n => {
            if (n === d) return 1;
            let isNeighbor = false;
            links.forEach(l => {
                if ((l.source.id === d.id && l.target.id === n.id) || (l.target.id === d.id && l.source.id === n.id)) {
                    isNeighbor = true;
                }
            });
    ...
    .on('mouseout', function(event, d) {
        ...
        link.style('stroke-opacity', 0.6).style('stroke', '#999').style('stroke-width', 1.5);
